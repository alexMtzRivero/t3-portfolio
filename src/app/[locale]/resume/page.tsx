...
export default function ResumePage() {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      `;
      header.innerHTML = `
        <h1 style="font-size: 36px; font-weight: bold; margin: 0 0 10px 0; color: #000000;">${t('hero.name')}</h1>
        <p style="font-size: 20px; color: #1e40af; margin: 0 0 20px 0; font-weight: 600;">${t('hero.title')} | Full-Stack Developer</p>
        <div style="display: flex; justify-content: center; gap: 30px; flex-wrap: wrap;">
          <span style="color: #000000;">${t('contact.methods.email.url').replace('mailto:', '')}</span>
          <span style="color: #000000;">${t('contact.methods.linkedin.name')}</span>
          <span style="color: #000000;">${t('contact.methods.github.name')}</span>
        </div>
      `;
      ...
      const skillsHTML = skillCategories.map(category => `
        <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; border: 1px solid #e5e7eb;">
          <h3 style="font-size: 18px; font-weight: bold; color: #1e40af; margin: 0 0 15px 0; text-align: center;">${category.title}</h3>
          <div style="display: flex; flex-direction: column; gap: 10px;">
            ${category.skills.map(skill => `
              <div style="display: flex; justify-content: space-between; align-items: center;">
                <span style="color: #000000; font-weight: 500;">${skill.name}</span>
                <span style="color: #6b7280; font-size: 14px;">${skill.level}%</span>
              </div>
            `).join('')}
          </div>
        </div>
